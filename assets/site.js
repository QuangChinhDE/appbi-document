/* ===== AppBI product site — shared shell (header / footer / changelog modal) ===== */
(function(){
  var MODULES = [
  ...
  var ddItems = MODULES.map(function(m){
    return '<a href="'+m.id+'.html"><span class="di">'+m.ic+'</span><span><b>'+m.n+'</b><small>'+m.d+'</small></span></a>';
  }).join('');
  var navActive = function(id){ return page===id ? ' class="active"' : ''; };
  var header =
    '<header class="site"><div class="container bar">'+
      ...
      '<nav id="nav">'+
        '<a href="index.html"'+navActive('index')+'>Tổng quan</a>'+
        '<div class="dd" id="dd"><button class="navbtn" id="ddbtn">Sản phẩm ▾</button>'+
          '<div class="dd-menu">'+ddItems+'</div></div>'+
        '<a href="showcase.html"'+navActive('showcase')+'>Sản phẩm đầu cuối</a>'+
        '<button class="navbtn" data-clog>Cập nhật</button>'+
  ...
  var footer =
    '<footer class="site"><div class="container">'+
      '<div class="row">'+
        ...
        '<nav><a href="index.html">Tổng quan</a>'+footCols+'<a href="showcase.html">Demo</a>'+
          '<a href="#" data-clog>Cập nhật</a></nav>'+
      '</div>'+
      '<small>AppBI — Nền tảng phân tích &amp; ứng dụng dữ liệu. Ảnh minh hoạ dùng bộ dữ liệu mẫu (chỉ là ví dụ).</small>'+
    '</div></footer>';
  ...
  var modal =
    '<div class="clog-overlay" id="clog"><div class="clog-modal" role="dialog" aria-modal="true">'+
      '<div class="clog-head"><div><h3>📜 Nhật ký cập nhật &amp; Lộ trình</h3>'+
        '<p>AppBI phát hành liên tục — cả nâng cấp lớn lẫn cải tiến nhỏ.</p></div>'+
        '<button class="clog-x" data-clog-close aria-label="Đóng">✕</button></div>'+
      '<div class="clog clog-body">'+
        '<div class="clog-note"><b>Ghi chú:</b> ngày &amp; người thực hiện mang tính minh hoạ cho demo — hãy chỉnh cho khớp thực tế.</div>'+
        '<div class="clog-legend"><span><b>Người:</b></span>'+
  ...
  /* ---------- behaviour ---------- */
  var dd=document.getElementById('dd'), ddbtn=document.getElementById('ddbtn');
  ...
  function openClog(){ clog.classList.add('open'); document.body.style.overflow='hidden'; }
  function closeClog(){ clog.classList.remove('open'); document.body.style.overflow=''; }
  document.addEventListener('click', function(e){
    if(e.target.closest('[data-clog]')){ e.preventDefault(); openClog(); document.getElementById('nav').classList.remove('open'); return; }
    if(e.target.closest('[data-clog-close]') || e.target===clog){ closeClog(); return; }
  });
  document.addEventListener('keydown', function(e){ if(e.key==='Escape' && clog.classList.contains('open')) closeClog(); });
